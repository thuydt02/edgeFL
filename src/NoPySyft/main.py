...
def main(args):
    if args.mode == 'federated-non-iid':
        trainer = Trainer(NUM_CLIENTS, LEARNING_RATE, BATCH_SIZE, NUM_EPOCHS, GLOBAL_UPDATE_AFTER_EVERY, w_DIR, acc_DIR)
        trainer.train()
    else:
        print("mode: hierarchical-non-iid")
        trainer = CloudServer(NUM_EDGE_SERVERS, NUM_CLIENTS, NUM_EPOCHS, BATCH_SIZE, LEARNING_RATE, EDGE_UPDATE_AFTER_EVERY, GLOBAL_UPDATE_AFTER_EVERY, w_DIR, z_DIR, acc_DIR)
        #trainer.pre_train_for_z_graph(LOCAL_EPORCHS_PRE_TRAIN)
        trainer.train()
