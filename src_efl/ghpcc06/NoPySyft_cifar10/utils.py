...
def partition_data_by_label(dataset, datadir, n_lables, n_parties):
    if dataset == 'mnist':
...
def partition_data_82_(dataset, datadir, n_parties, dominate_class_ratio = 0.8):
    K = 10 # num_labels
    if dataset == 'mnist':
        X_train, y_train, X_test, y_test = load_mnist_data(datadir)
    elif dataset == 'fmnist':
    ...
    for i in range(n_parties):
        print("i, len of set_not_selected: ", i, ", ", len(set_not_selected))
        
        dominant = set([])
        tmp = set_not_selected.copy()
        lstar = -1
        for l in range (K):
            if len(label_ind[l]) > num_dominant + num_minor * (n_parties - i + 1) / K:
                tmp = tmp - set(label_ind[l])
                print('l, len(label_ind[l]): ', l, " ", len(label_ind[l]))
                dominant = set(label_ind[l][0:num_dominant])
                label_ind[l] = list(set(label_ind[l]) - set(dominant))
                print('l, len(label_ind[l]): ', l, " ", len(label_ind[l]))
        ...
        minor = random.sample(tmp, num_minor)

        for k in range(K):
            if k == lstar:
                continue
            label_ind[k] = list(set(label_ind[k]) - set(minor))


        set_not_selected = set_not_selected - set(minor)
        
        net_dataidx_map[i] = np.append(list(dominant), list(minor))
