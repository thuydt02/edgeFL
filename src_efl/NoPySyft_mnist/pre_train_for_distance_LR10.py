...
class pre_train:
    def __init__(self, n_clients, global_rounds, batch_size, learning_rate, lr_decay, z_dir, w_dir,
        ...
        self.model =  LR10().to(device)#CNN1().to(device) #MLP2().to(device) #MNIST().to(device) #CNNModel().to(device) MNIST FEMNIST
        summary(self.model, (1, 28, 28))
        self.model_name = "LR10"
    ...
    def get_distance_and_weight_files(self):
        ...
        if (self.partition_data_file != None):
            ...
            self.train_data = self.data_loader.prepare_in_batch_given_partition_LR10(len(self.clients), self.z_dir + self.partition_data_file)
