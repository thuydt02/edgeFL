...
class ClientNode:
	...
	def train(self, device, lr):
		...
		for batch_idx, (images, labels) in enumerate(self.data):
			...
			loss = self.model["criterion"](output.float(), labels.unsqueeze(1).float())
	...
	def train_stats(self, device):
		...
		with torch.no_grad():
			for batch_idx, (images, labels) in enumerate(self.data):
				...
				corrects += output.eq(labels.unsqueeze(1).view_as(output)).sum().item()
				
				loss += self.model["criterion"](output.float(), labels.unsqueeze(1).float()).item()
