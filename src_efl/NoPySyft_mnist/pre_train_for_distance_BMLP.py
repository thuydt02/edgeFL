...
#from client_nodeLR import ClientNode
#from mnist_model import BMLP2
...
class pre_train:
    ...
    def get_distance_and_weight_files(self):
        ...
        if (self.partition_data_file != None):
            ...
            self.train_data = self.data_loader.prepare_in_batch_given_partition(len(self.clients), self.z_dir + self.partition_data_file) #y_train = 0 -> 9
            #self.train_data = self.data_loader.prepare_in_batch_given_partition_LR(len(self.clients), self.z_dir + self.partition_data_file) #y_train = 0 or 1
    ...
    def get_distance_and_weight_files_in_round_steps(self, step):
        ...
        for epoch in range(1, self.global_rounds + 1):
            print(f"Epoch {epoch}/{self.global_rounds}")
            for i, client in tqdm(enumerate(self.clients)):
                client.train(device, lr)
            #lr = lr * self.lr_decay
            ...
            if (epoch % step == 0):
                ...
                self.create_distance_file(step = epoch)
                self.create_client_wPCA2D_file(step = epoch)
    ...
    def create_client_wPCA2D_file(self, step = -1):
        #create client weights by PCA in 2D file
        if step == -1:
            step = self.global_rounds
        W = []
        ...
        wPCA_fname = "wPCA_" + self.model_name + "_G" + str(self.global_rounds) + "_round"+  str(step) +"_" + self.partition_data_file

        principalDf.to_csv(self.z_dir + wPCA_fname)
        print("saved in file: ", self.z_dir + wPCA_fname)

        
    def create_distance_file(self, step = -1):
        ...
        if self.p == 2:  
            d_fname = "d_euclidean_" +self.model_name + "_G" + str(self.global_rounds) + "_round"+  str(step) + "_" + self.partition_data_file 
        else:
            d_fname = "d_" + self.distance_metric + "_p" + str(self.p) + "_" +self.model_name + "_G" + str(self.global_rounds) + "_round"+  str(step) + "_" + self.partition_data_file
