...
class pre_train:
    def __init__(self, n_clients, global_rounds, batch_size, learning_rate, lr_decay, z_dir, w_dir,
        ...
        self.model =  LR().to(device)#CNN1().to(device) #MLP2().to(device) #MNIST().to(device) #CNNModel().to(device) MNIST FEMNIST
    ...
    def get_distance_and_weight_files(self):
        ...
        if (self.partition_data_file != None):
            ...
            self.train_data = self.data_loader.prepare_in_batch_given_partition_LR(len(self.clients), self.z_dir + self.partition_data_file)
        ...
        #self.create_client_wTSNE2D_file()
        self.create_client_model_file()
    ...
    def create_client_wTSNE2D_file(self):
        #create client weights by TSNE in 2D file
        W = []
        for i in tqdm(range(len(self.clients))):
        ...
        print("type of W[0][0] ", type(W[0][0]))
        
        tsne = TSNE(n_components = 2, init = "random", random_state = 42)

        W2D = tsne.fit_transform(W)
        W2DlDf = pd.DataFrame(data = W2D, columns = ['pc1', 'pc2'])
        ...
        wTSNE_fname = "wTSNE_" + self.model_name + "_G" + str(self.global_rounds) + "_" + self.partition_data_file

        W2DlDf.to_csv(self.z_dir + wTSNE_fname)
        print("saved in file: ", self.z_dir + wTSNE_fname)

    def create_client_model_file(self):
        ...
        Wdf = pd.DataFrame(data = W)

        
        if not os.path.exists(self.z_dir):
            os.makedirs(self.z_dir)
        
        w_fname = "weight_" + self.model_name + "_G" + str(self.global_rounds) + "_" + self.partition_data_file

        Wdf.to_csv(self.z_dir + w_fname, index = False)
        print("saved in file: ", self.z_dir + w_fname)
