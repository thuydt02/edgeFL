...
#from eFL import eFL
from fl_tpu import Trainer

#NUM_EDGE_SERVERS = 8
NUM_CLIENTS = 100
BATCH_SIZE = 10
LEARNING_RATE = 0.001 #should be 0.1 


GLOBAL_TIME = 100 # global for-loop interations 500 for FL, lr=0.001 for 156

# number of local client updates before sending model up to server (in FL) or to edge (in eFL)
NUM_LOCAL_UPDATES_BEFORE_SENDING_UP = 1

# the following parameters are used for Edge FL
#NUM_EDGE_UPDATES_BEFORE_SENDING_UP = 10
...
def main(args):
    if args.mode == 'FL-non-iid':
        trainer = Trainer(NUM_CLIENTS, LEARNING_RATE, BATCH_SIZE, GLOBAL_TIME, NUM_LOCAL_UPDATES_BEFORE_SENDING_UP, w_DIR, acc_DIR, MODE, ZIPF_Z)
        trainer.run_train()
    else:
