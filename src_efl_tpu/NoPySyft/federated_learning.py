...
class Trainer:
	def __init__(self, no_clients, learning_rate, batch_size, epochs, no_local_epochs, w_dir, acc_dir, mode, zipf_z):
		self.model = CNNModel().to(device)#WRAPPED_MODEL.to(device)#
		...
		self.w_file = "FL_TPU" + mode 
		
		if zipf_z != None:
			self.w_file = self.w_file + "_zipfz" + str(self.zipfz)

		self.w_file = self.w_file + "_B" + str(batch_size) + "_L" + str(no_local_epochs) + "_G" + str(int(epochs*no_local_epochs)) + ".weight.pth"
